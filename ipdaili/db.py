...
class RedisClient(object):
    ...
    def __init__(self, host=REDIS_HOST, port=REDIS_PORT, psw=REDIS_PASSWORD):
        try:
            self.redisdb = redis.StrictRedis(host=host, port=port, password=psw, decode_responses=True)
            print('redis连接成功......')
        except:
            print('连接失败')
